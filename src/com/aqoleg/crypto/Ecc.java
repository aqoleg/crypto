...
public class Ecc {
    ...
    private Ecc(BigInteger a, BigInteger b, BigInteger p) {
        if (four.multiply(a.pow(3)).add(BigInteger.valueOf(27).multiply(b.pow(2))).signum() == 0) {
            throw new Exception("the curve is singular, requires 4 * a**3 + 27 * b**2 != 0");
        }
        if (!p.isProbablePrime(10)) {
            throw new Exception("this is not a field, requires prime p");
        }
        this.a = a;
        this.b = b;
        this.p = p;
     ...
     * @throws Ecc.Exception        if 4 * a**3 + 27 * b**2 == 0 or p is non-prime
     */
     ...
     * @throws Ecc.Exception        if 4 * a**3 + 27 * b**2 == 0 or p is non-prime or g * n != infinity
     */
    public static Ecc getEcc(
        ...
        if (!curve.gMultiply(n).isInfinity()) {
            throw new Exception("incorrect n for this base point, requires g * n = infinity");
        }
        return curve;
    }
    ...
    public Point createPoint(BigInteger x, BigInteger y) {
        if (x.signum() < 0 || x.compareTo(p) >= 0) {
            throw new Exception("x is not in the field Fp, requires 0 <= x < p");
        }
        if (y.signum() < 0 || y.compareTo(p) >= 0) {
            throw new Exception("y is not in the field Fp, requires 0 <= y < p");
        }
        // y**2 % p = (x**3 + a * x + b) % p
     ...
     * @throws Ecc.Exception        if x is not in the field Fp or point is not exist or it is not on the curve
     */
    public Point createPoint(BigInteger x, boolean even) {
        ...
        if (legendre(r) != 1) {
            throw new Exception("there is no point with this x");
        }
     ...
     * @throws Ecc.Exception        if message is too big
     */
    public BigInteger[] sign(byte[] message, BigInteger d) {
        BigInteger z = new BigInteger(1, message);
        if (z.bitLength() > n.bitLength()) {
            throw new Exception("the message is too big, requires bitLength <= n.bitLength");
        }
        BigInteger r;
